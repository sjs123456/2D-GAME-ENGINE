# -*- coding: utf-8 -*-
"""TC05b 快速复核（v4 轮）：v3 已存在账号 testuser_v3_202608062115081500 注册应返回 400/提示已被占用
与 recheck-tc05b-v3.py 同构，仅换账号与截图版本。"""
...
USER = "testuser_v3_202608062115081500"
SUFFIX = time.strftime("%Y%m%d%H%M%S") + str(time.time_ns() % 10000)
EMAIL = f"tc05b_{SUFFIX}@test.com"
shot = os.path.join(OUT_DIR, "register-TC05b-v4.png")
...
with sync_playwright() as p:
    ...
    verdict = "PASS" if ("已被占用" in body or "已存在" in body or any(400 <= s < 500 for s, _ in api)) else "FAIL"
    print("账号:", USER)
    print("邮箱:", EMAIL)
    print("API:", api)
