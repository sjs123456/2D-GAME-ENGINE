# -*- coding: utf-8 -*-
"""按 generate-test-report skill 生成注册功能第二轮复测报告"""
...
OUT = os.path.join(WS, "自动化测试平台-注册页自动化测试报告-v2.html")  # 显式绝对路径
...
with open(os.path.join(TMP, "register-test-results-v2.json"), encoding="utf-8") as f:
    results = json.load(f)
...
html = f"""<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"/><title>自动化测试平台-注册页自动化测试报告（第二轮复测）</title>
<style>
...
<body><div class="wrap">
<header>
<h1>自动化测试平台 · 注册页面自动化测试报告（第二轮复测）</h1>
...
<p>复测说明：与首轮（2026-08-06 20:48）间隔约 7 分钟，用例集与判定逻辑保持一致，对比验证结果稳定性与已知缺陷修复情况</p>
</header>
<div class="cards">
...
<h2>一、测试目标与范围</h2>
<p>对注册功能进行<b>第二轮复测（回归测试）</b>：使用与首轮完全一致的 12 条用例（正常注册、密码一致性、密码强度、用户名唯一性/长度、邮箱格式、必填项空值、注册登录闭环），验证功能行为稳定性，并确认首轮发现的 3 个缺陷（OBS-001/002/003）是否修复。每条用例使用独立浏览器上下文隔离状态，记录提示文案与后端 API 响应，每步保存截图。</p>
<h2>二、注册页字段分析</h2>
<p>注册页与登录页共用 <code>/login</code> URL，通过 <b>tab 切换</b>（<code>button.tab-btn</code>）进入，表单 <code>form.login-form</code>，<b>无验证码、无协议勾选</b>。提示形式：<code>el-message</code> toast。</p>
<table>
...
{img_tag(os.path.join(TMP, "register-page.png"), "注册页截图（首轮，本轮结构未变）")}
<h2>三、执行过程（步骤级记录）</h2>
{steps}
<h2>四、用例明细汇总</h2>
...
<h2>六、与首轮对比（稳定性结论）</h2>
<div class="good">
✅ <b>结果稳定：12/12 条用例 verdict 与首轮完全一致（12 同 0 异）</b>，无 BLOCKED/FAIL，功能行为复现稳定。<br/>
✅ 补充验证：首轮已存在账号 <code>testuser_202608062048216900</code> 再次注册 → 400「用户名已被占用」，与 TC05 主用例（admin123）行为一致，双重确认唯一性拦截正常。
</div>
<h2>七、缺陷复测确认（均仍存在，未修复）</h2>
<div class="warn">
<b>OBS-001（中）｜前端缺格式本地校验，Pydantic 英文错误透传 —— 仍复现</b><br/>
TC03 实测：<code>password: String should have at least 8 characters</code>（英文透传）<br/>
TC06 实测：<code>username: String should have at least 3 characters</code>（英文透传）<br/>
TC04 实测：<code>password: Value error, 密码必须包含大写字母</code>（Pydantic 前缀透传，内容中文）<br/>
均先发请求、后端 422 后才提示，前端无本地格式校验。
</div>
<div class="note">
<b>OBS-002（低）｜邮箱 HTML5 英文气泡 —— 仍复现</b><br/>
TC07 实测：<code>Please include an '@' in the email address. 'not-an-email' is missing an '@'.</code>
</div>
<div class="note">
<b>OBS-003（低）｜确认密码为空提示「两次输入的密码不一致」—— 仍复现</b><br/>
TC11 实测：提示「两次输入的密码不一致」，语义应改为「请再次输入密码」。
</div>
<h2>八、测试结论与建议</h2>
<div class="good">
✅ 注册核心链路稳定可用：注册（201）→ 自动切回登录 tab → 新账号登录（200）→ /dashboard 闭环通过；前端必填校验与密码一致性校验正确；两轮 24 次执行无功能级失败。<br/>
⚠️ 3 个已知缺陷（OBS-001/002/003）经复测确认均未修复，建议纳入缺陷跟踪排期。
</div>
<p style="line-height:1.8;"><b>建议：</b><br/>
1. 将 OBS-001/002/003 纳入缺陷跟踪，修复后安排第三轮回归；<br/>
2. 复测新账号 <code>testuser_retest_202608062055097400</code> 为真实注册（闭环验证用），如需清理请管理员处理（连同首轮 <code>testuser_202608062048216900</code>）；<br/>
3. 建议将两轮用例集固化进 CI，作为注册功能回归基线。</p>
