# -*- coding: utf-8 -*-
"""登录功能测试：http://123.56.21.178:8080/login
TC01-TC06，每条用例独立 browser context 隔离登录态。
通过 MutationObserver 捕获短暂 toast 提示，监听 dialog 与 login API 响应，轮询 URL/标题。
"""
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for case in CASES:
            ...
            shot = os.path.join(OUT_DIR, f"login-{case['id']}.png")
            rec["screenshot"] = shot
            page = None
            context = None
            try:
    ...
    # ---- 汇总表 ----
    print("\n" + "=" * 120)
    print("登录功能测试汇总表  http://123.56.21.178:8080/login")
    print("=" * 120)
    ...
    out_json = os.path.join(OUT_DIR, "login-test-results.json")
