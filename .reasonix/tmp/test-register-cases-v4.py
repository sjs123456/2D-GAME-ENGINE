# -*- coding: utf-8 -*-
"""注册功能第四轮复测（回归测试）：http://123.56.21.178:8080/login (注册 tab)
与上轮 register-test-results-v3.json 完全相同的 12 条用例与判定逻辑，验证结果稳定性与缺陷复现。
- 每条用例独立 browser context，MutationObserver 捕获 toast，page.on("response") 记录 API 状态码
- 输出: register-test-results-v4.json（字段与上轮完全一致）+ register-TC{NN}-v4.png
- 末尾与上轮 v3 verdict 逐条对比，并复核 OBS-001/002/003
"""
...
PREV_JSON = os.path.join(OUT_DIR, "register-test-results-v3.json")
NEW_JSON = os.path.join(OUT_DIR, "register-test-results-v4.json")
...
REG_USER = f"testuser_v4_{SUFFIX}"   # testuser_v4+时间戳（本轮第四轮，符合字母数字下划线规则）
REG_EMAIL = f"{REG_USER}@test.com"
REG_PWD = "Test123456"
PREV_EXIST_USER = "testuser_v3_202608062115081500"  # 上轮(v3)已存在账号，补充验证
...
def main():
    print(f"本轮唯一测试账号(v4): {REG_USER} / {REG_EMAIL} / {REG_PWD}")
    print(f"上轮(v3)已存在账号(补充验证): {PREV_EXIST_USER}\n")
    results = []
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        for case in CASES:
            if case["id"] == "TC12":
                rec = run_login_verify(browser, case, os.path.join(OUT_DIR, "register-TC12-v4.png"),
                                       REG_USER, REG_PWD)
            else:
                rec = run_register_case(browser, case, os.path.join(OUT_DIR, f"register-{case['id']}-v4.png"))
            results.append(rec)
            print(f"[{rec['id']}] verdict={rec['verdict']} | {rec['result'][:200]}")
        # ---- TC05 补充验证：上轮已存在账号 testuser_202608062048216900 ----
        print("\n[补充] TC05b 用户名已存在(上轮账号 testuser_v3_202608062115081500 补充验证)：")
        extra = dict(CASES[4])
        extra["id"] = "TC05b"
        extra["desc"] = "用户名已存在（v3已存在账号补充验证）"
        extra["user"] = PREV_EXIST_USER
        extra["email"] = f"tc05b_{SUFFIX}@test.com"
        rec5b = run_register_case(browser, extra, os.path.join(OUT_DIR, "register-TC05b-v4.png"))
        print(f"  [{rec5b['id']}] verdict={rec5b['verdict']} | {rec5b['result'][:200]}")
        browser.close()

    # ---- 汇总表 ----
    print("\n" + "=" * 130)
    print("注册功能第四轮复测汇总表  http://123.56.21.178:8080/login (注册 tab)")
    print("=" * 130)
    ...
    try:
        ...
        print("-" * 90)
        print(f"12条 verdict 与 v3 全部一致: {all_same}")
    except Exception as e:
        print(f"读取上轮结果失败: {e}")
