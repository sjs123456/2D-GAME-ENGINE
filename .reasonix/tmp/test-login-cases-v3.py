# -*- coding: utf-8 -*-
"""登录功能测试（第三轮）：http://123.56.21.178:8080/login
基于 v2 test-login-cases-v2.py 复用，仅修改动态部分：
  - 输出 JSON 版本号 v3（login-test-results-v3.json）
  - 截图版本后缀 -v3（login-TC{NN}-v3.png）
  - script 字段 test-login-cases-v3.py
用例输入与判定逻辑与 v1/v2 完全一致。
"""
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for case in CASES:
            rec = {"id": case["id"], "desc": case["desc"], "expect": case["expect"],
                   ...
                   "script": "test-login-cases-v3.py", "run_at": run_at,
                   "step": f"输入账号 {case['user']!r} + 密码 {case['pwd']!r}，点击登录"}
            shot = os.path.join(OUT_DIR, f"login-{case['id']}-v3.png")
            rec["screenshot"] = shot
            page = None
            context = None
            try:
    ...
    # ---- 汇总表 ----
    print("\n" + "=" * 120)
    print("登录功能测试汇总表(v3)  http://123.56.21.178:8080/login")
    print("=" * 120)
    ...
    out_json = os.path.join(OUT_DIR, "login-test-results-v3.json")
