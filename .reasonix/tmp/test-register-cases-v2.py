# -*- coding: utf-8 -*-
"""注册功能第二轮复测（回归测试）：http://123.56.21.178:8080/login (注册 tab)
...
- 末尾与上轮 verdict 逐条对比，并复核 OBS-001/002/003
"""
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for case in CASES:
            if case["id"] == "TC12":
                rec = run_login_verify(browser, case, os.path.join(OUT_DIR, "register-TC12-v2.png"),
                                       REG_USER, REG_PWD)
            else:
                rec = run_register_case(browser, case, os.path.join(OUT_DIR, f"register-{case['id']}-v2.png"))
            results.append(rec)
        ...
        rec5b = run_register_case(browser, extra, os.path.join(OUT_DIR, "register-TC05b-v2.png"))
        print(f"  [{rec5b['id']}] verdict={rec5b['verdict']} | {rec5b['result'][:200]}")
        browser.close()

    # ---- 汇总表 ----
    print("\n" + "=" * 130)
    print("注册功能第二轮复测汇总表  http://123.56.21.178:8080/login (注册 tab)")
    print("=" * 130)
